...
- 新增「合并网格」开关，默认开启；关闭后仅合并材质，直接复用来源原网格，`MergedMesh` 为 null，不生成、覆盖或删除已有合并网格文件。调用方需按原网格组织绘制并更新旧布局引用；来源索引保持全局来源列表下标，不改成组内索引。
- 来源列表即时显示按 Mesh 引用去重后的网格种数、合并后的顶点数与三角形数；添加、替换、移除、清空及 Undo 后按当前输入更新。空引用不计入，非三角形子网格提示统计不完整；统计不读取完整顶点或索引数组。
- 两种网格模式共用同一套 Shader，通过材质 Float 参数 `_HmMeshMergeFilterVertices` 控制顶点筛选：合并网格为 1，仅合并材质为 0，不新增 Shader 或关键字变体。模板、来源 Shader 注入副本和 URP 示例同步接入三参数 `HmMeshMergeIsMeshVisible`；自定义 Shader 必须声明并使用该参数，旧双参数函数保留原行为。
- 仅合并材质时不再要求原网格具有空闲索引通道，也不受顶点色索引的 256 种网格限制；来源 Shader 注入时复用已有对应语义，避免重复声明。材质 LUT、纹理数组与来源映射表继续共用，原有「尝试修改来源shader(副本)」的材质转换限制不变。
- 切换模式后需重新执行合并；游戏侧 renderObject 与布局不由插件自动调整。本次通过 C# 语法静态检查与 `git diff --check`，未执行 Unity 编译、导入或运行。
- 生成 Shader 的名字不再带配置 GUID：改为「HmMeshMerge/配置名」，只有工程里存在同名配置时才按资产路径顺序补 1 起的序号。哈希名既不可读也不便于提交，序号在重复合并时保持稳定。
- 新增「尝试修改来源shader(副本)」开关：勾选后复制来源 Shader 文本并注入合并接入点（_MeshMergeIndex / _HmMeshMergeParams / _HmMeshMergeSources 三个属性、脚本包含与两张查找纹理声明、索引通道与实例化输入、nointerpolation 材质索引插值通道、每个顶点入口的可见性包装），保留风动、光照、Alpha 裁剪等自有逻辑；不勾选仍生成 URP 无光照模板。
- 复制路径不自动改写逐来源不同的数值引用与贴图取样：不生成纹理数组、材质使用第一来源的贴图与数值，需要逐来源时按生成文件头部注释的待办改用 HmMeshMergeLoadParam（含行号）与 2DArray 采样；UsePass 引用的 Pass 注入不到（隐藏来源仍会绘制），一并列入待办。
- 复制路径在结构上定位不到必需元素时报错而不产出半成品：没有同时带 POSITION 与 SV_POSITION 的结构体、没有 Properties 块、索引通道与源 Shader 已用语义冲突、顶点入口的参数或返回结构体不是注入过通道的那两个、来源不是工程内的 .shader 资产。
- 两条 Shader 路径都在合并前校验所有来源使用同一个 Shader，不一致直接报错。
