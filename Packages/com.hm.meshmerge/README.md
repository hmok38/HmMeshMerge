...
- 生成 Shader 在文件里的名字是 `HmMeshMerge/{配置名}`（Shader 名，不是文件名）；工程里存在同名配置时按资产路径顺序补 1 起的序号，避免同名 Shader 互相顶替。用序号而不是 GUID 或时间戳，名字可读、可提交，重新合并也不会改名。
...
勾选「尝试修改来源shader(副本)」后，工具不再生成模板，而是读取来源 Shader 的文本、注入合并接入点后另存为 `{配置名}_Shader.shader`。**源 Shader 文件只被读取，不会被修改**；生成的是副本，每次合并都会覆盖，要长期修改请先复制成自有 Shader。
...
完成源码静态检查；未执行 Unity 编译、导入、测试或运行。请手动重新合并，检查切换来源、参数值、阴影/深度和二次合并后的已有引用，并确认同一网格的几何只剩一份、同一材质只占一层贴图。顶点着色器会用 Load 读来源映射表，请在目标平台确认顶点纹理获取可用。勾选「尝试修改来源shader(副本)」时还要确认注入后的 Shader 能编译、风动等自有逻辑保留、被隐藏的来源不投影，并逐条处理生成文件头部列出的待办。
